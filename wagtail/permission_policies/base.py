...
class BaseDjangoAuthPermissionPolicy(BasePermissionPolicy):
    ...
    def _get_users_with_any_permission_codenames_filter(self, permission_codenames):
        ...
        groups_filter = Q(
            groups__permissions__content_type=self._content_type,
            groups__permissions__codename__in=permission_codenames,
        )
        user_filter = Q(
            user_permissions__content_type=self._content_type,
            user_permissions__codename__in=permission_codenames,
        )
        return Q(groups_filter | user_filter) | Q(is_superuser=True)
...
class OwnershipPermissionPolicy(BaseDjangoAuthPermissionPolicy):
    ...
    * ability to edit also implies ability to delete
    ...
    def user_has_permission(self, user, action):
        ...
        if action == "change":
            return (
                ...
                or user.has_perm(self._get_permission_name("add"))
            )
        if action == "delete":
            return (
                user.has_perm(self._get_permission_name("delete"))
    ...
    def users_with_any_permission_for_instance(self, actions, instance):
        if "change" in actions or "delete" in actions:
            if self.user_has_any_permission_for_instance(
                instance.owner, actions, instance
            ):
